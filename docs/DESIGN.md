...
**Version:** 0.10
**Status:** Active design; pre-1.0 feature set and administrator console implemented
**Targets:** Web, Windows, macOS
**Primary client language:** TypeScript
**Desktop framework:** Tauri 2
**Cloud database:** PostgreSQL
**Desktop/local data:** SQLite planned; IndexedDB used by the current web-capable iteration

### Current implementation checkpoint (v0.10)

The current repository now implements the core connected personal-library layer: sortable grid/table browsing, a single reading status plus many user-defined shelves, series/volume, half-star ratings, book descriptions/reviews, multiple read dates/rereads, Goodreads and LibraryThing imports, JSON backup/restore, catalog/local covers with device caching, categorized statistics, accounts, PostgreSQL cloud persistence, desktop SQLite and web/desktop synchronization.
...
BookStats/
├── apps/
│   ├── client/            # Normal React/Vite + Tauri shell
│   ├── admin/             # Separate administrator-only React/Vite client
│   └── server/            # Fastify normal + protected administrator API
├── packages/
...
Matching priority for ordinary imports: prior source ID, ISBN-13, ISBN-10, catalog ID, title+author+edition, then fuzzy candidates that require confirmation. LibraryThing is a deliberate exception because its `books_id` represents an individual user catalog entry/copy: the same `books_id` updates on re-import, while different LibraryThing entries stay separate even when they share an ISBN or title + author.
...
The administrator surface is a separate web application and is not linked or imported by the normal user client. Administrative authority is enforced server-side on every protected route. Role promotion is a server-command operation only; there is no HTTP endpoint that can grant administrator privileges. Destructive administrator actions are explicit and audited. The browser administrator console deliberately cannot execute arbitrary SQL, shell commands, systemd operations or NGINX changes.
...
### Milestone 4 — Imports
Import preview/undo plus the existing Goodreads and LibraryThing adapters, followed by legacy BookStats and generic CSV mapping.
...
The first connected BookStats milestone is now implemented.

Implemented from the roadmap:

- Open Library metadata search by ISBN/title/author through a provider-facing API layer
- cover and description enrichment
- editable metadata provenance on local Book records
- optional user accounts
- PostgreSQL-backed sessions and cloud library persistence
- local-first web storage with IndexedDB
- native desktop local storage with SQLite through Tauri
- migration from the v0.2 desktop WebView IndexedDB into SQLite
- web/desktop synchronization using stable Book UUIDs, tombstones, revisions and sync cursors

The v0.3 sync table intentionally stores the complete current `Book` payload as JSONB. This is an implementation bridge, not a reversal of the normalized Work/Edition/UserBook design above. It lets the present application synchronize reliably while the richer catalog/edition model continues to evolve.

Still deferred from the complete architecture:

- field-level conflict resolution for simultaneous edits
- secure desktop OS-keychain storage for login sessions
- email verification and password reset
- richer edition selection and alternate-cover selection
- full Work/Edition conversion of current client records
- Goodreads/LibraryThing/legacy BookStats import adapters
