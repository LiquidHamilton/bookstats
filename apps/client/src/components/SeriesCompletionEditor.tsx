...
export function SeriesCompletionEditor({ series, onSave, onClose }: Props) {
  ...
  const adjusted = Boolean(existing) || expectedCount !== automaticCount || includedIds.length !== automaticIds.length || includedIds.some((id) => !automaticIds.includes(id)) || manualBooks.length > 0;
