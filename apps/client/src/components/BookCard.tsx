...
export function BookCard({ book, shelfNames = [], onOpen, onEdit, onDelete, selectable = false, selected = false, onSelect }: Props) {
  const seriesLabel = book.series ? `${book.series}${book.seriesVolume ? ` · #${book.seriesVolume}` : ""}` : null;
  const assignedShelves = shelfNames;
  return (
    <article className={`book-card ${selected ? "selected" : ""}`}>
      ...
      <button className="cover-button" onClick={() => onOpen(book)} aria-label={`Open ${book.title}`}>
