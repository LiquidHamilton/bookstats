...
async function createSession(client, userId) {
    ...
    await client.query(`INSERT INTO sessions (user_id, token_hash, expires_at) VALUES ($1, $2, now() + ($3 || ' days')::interval)`, [userId, sha256(token), ADMIN_SESSION_DAYS]);
    return token;
}
async function issueResetToken(client, userId) {
    const token = randomBytes(32).toString("base64url");
    await client.query("UPDATE password_reset_tokens SET used_at = now() WHERE user_id = $1 AND used_at IS NULL", [userId]);
    await client.query(`INSERT INTO password_reset_tokens (user_id, token_hash, expires_at) VALUES ($1, $2, now() + ($3 || ' hours')::interval)`, [userId, sha256(token), RESET_HOURS]);
    return token;
}
async function writeAudit(client, request, action, targetUserId, targetRecordId, details = {}) {
    const actor = request.bookstatsAdmin;
    if (!actor)
        throw new Error("Administrator context is missing.");
    await client.query(`INSERT INTO admin_audit_log (admin_user_id, admin_email, action, target_user_id, target_record_id, details, ip_address)
     VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7)`, [actor.id, actor.email, action, targetUserId ?? null, targetRecordId ?? null, JSON.stringify(details), request.ip]);
}
async function withAuditTransaction(pool, request, action, targetUserId, targetRecordId, details, operation) {
...
export function registerAdminRoutes(app, pool, appVersion) {
    const adminLoginWindows = new Map();
    const requirePool = (reply) => {
        if (pool)
            return pool;
    ...
    const authenticateAdmin = async (request, reply) => {
        ...
        const result = await db.query(`SELECT u.id, u.email, u.display_name, u.password_hash, u.role, u.disabled_at, u.created_at
       FROM sessions s JOIN users u ON u.id = s.user_id
       WHERE s.token_hash = $1 AND s.expires_at > now() AND u.role = 'admin' AND u.disabled_at IS NULL`, [tokenHash]);
        const row = result.rows[0];
        if (!row)
            return reply.code(401).send({ error: "Administrator session expired or invalid." });
    ...
    app.post("/api/v1/admin/auth/login", async (request, reply) => {
        ...
        if (attempts.length >= 8)
            return reply.code(429).send({ error: "Too many administrator sign-in attempts. Try again later." });
        attempts.push(now);
        adminLoginWindows.set(request.ip, attempts);
        const client = await db.connect();
        try {
            const result = await client.query(`SELECT id, email, display_name, password_hash, role, disabled_at, created_at
    ...
    app.post("/api/v1/admin/auth/logout", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db || !request.bookstatsAdmin)
            return;
        const client = await db.connect();
        try {
            await client.query("BEGIN");
            await writeAudit(client, request, "ADMIN_LOGOUT", undefined, undefined, {});
            if (request.bookstatsAdminTokenHash)
                await client.query("DELETE FROM sessions WHERE token_hash = $1", [request.bookstatsAdminTokenHash]);
            await client.query("COMMIT");
            return { ok: true };
        }
        catch (error) {
    ...
    app.get("/api/v1/admin/dashboard", { preHandler: authenticateAdmin }, async (_request, reply) => {
      ...
      pg_database_size(current_database())::text AS database_bytes`);
        ...
        const row = result.rows[0];
        const recent = await db.query(`SELECT id, email, display_name, created_at FROM users ORDER BY created_at DESC LIMIT 8`);
        return {
            metrics: {
    ...
    app.get("/api/v1/admin/users", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        ...
        const search = `%${q}%`;
        const count = await db.query(`SELECT count(*) FROM users WHERE $1 = '%%' OR email ILIKE $1 OR display_name ILIKE $1`, [search]);
        const result = await db.query(`SELECT u.id, u.email, u.display_name, u.role, u.disabled_at, u.email_verified_at, u.created_at,
    ...
    app.get("/api/v1/admin/users/:userId", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        const result = await db.query(`SELECT id, email, display_name, role, disabled_at, email_verified_at, created_at, updated_at FROM users WHERE id = $1`, [request.params.userId]);
        const row = result.rows[0];
        if (!row)
    ...
    app.patch("/api/v1/admin/users/:userId", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        ...
        if (request.body?.displayName !== undefined && !displayName)
            return reply.code(400).send({ error: "Display name cannot be blank." });
        if (request.body?.email !== undefined && !email)
            return reply.code(400).send({ error: "Enter a valid email address." });
        try {
            const updated = await withAuditTransaction(db, request, "USER_PROFILE_UPDATE", request.params.userId, undefined, { displayNameChanged: request.body.displayName !== undefined, emailChanged: request.body.email !== undefined, verificationChanged: request.body.emailVerified !== undefined }, async (client) => {
                const existing = await client.query("SELECT email, display_name, email_verified_at FROM users WHERE id = $1 FOR UPDATE", [request.params.userId]);
                if (!existing.rows[0])
                    throw Object.assign(new Error("User not found."), { statusCode: 404 });
                const nextEmail = email ?? existing.rows[0].email;
                const nextName = displayName ?? existing.rows[0].display_name;
                let verifiedAt = existing.rows[0].email_verified_at;
                if (email && email !== existing.rows[0].email)
                    verifiedAt = null;
                if (request.body.emailVerified === true)
                    verifiedAt = new Date();
                if (request.body.emailVerified === false)
                    verifiedAt = null;
                const result = await client.query(`UPDATE users SET email=$1, display_name=$2, email_verified_at=$3, updated_at=now() WHERE id=$4
             RETURNING id, email, display_name, role, disabled_at, email_verified_at, created_at, updated_at`, [nextEmail, nextName, verifiedAt, request.params.userId]);
    ...
    app.post("/api/v1/admin/users/:userId/disable", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db || !request.bookstatsAdmin)
            return;
        if (request.params.userId === request.bookstatsAdmin.id)
            return reply.code(400).send({ error: "You cannot disable your own administrator account." });
    ...
    app.post("/api/v1/admin/users/:userId/enable", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
    ...
    app.post("/api/v1/admin/users/:userId/invalidate-sessions", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db || !request.bookstatsAdmin)
            return;
        if (request.params.userId === request.bookstatsAdmin.id)
            return reply.code(400).send({ error: "Use Sign out to end your own administrator session." });
    ...
    app.post("/api/v1/admin/users/:userId/force-password-reset", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db || !request.bookstatsAdmin)
            return;
        if (request.params.userId === request.bookstatsAdmin.id)
            return reply.code(400).send({ error: "Use the normal account password-change flow for your own administrator account." });
        if (!emailConfigured())
            return reply.code(503).send({ error: "Transactional email is not configured, so BookStats cannot deliver a reset link." });
        let target;
        let token = "";
        await withAuditTransaction(db, request, "FORCE_PASSWORD_RESET", request.params.userId, undefined, {}, async (client) => {
            const result = await client.query("SELECT email, display_name FROM users WHERE id=$1 FOR UPDATE", [request.params.userId]);
    ...
    app.delete("/api/v1/admin/users/:userId/cloud-library", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        const user = await db.query("SELECT email FROM users WHERE id=$1", [request.params.userId]);
        if (!user.rows[0])
            return reply.code(404).send({ error: "User not found." });
        const expected = `CLEAR ${user.rows[0].email}`;
        if (request.body?.confirmation !== expected)
            return reply.code(400).send({ error: `Confirmation must exactly match: ${expected}` });
    ...
    app.delete("/api/v1/admin/users/:userId", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db || !request.bookstatsAdmin)
            return;
        if (request.params.userId === request.bookstatsAdmin.id)
    ...
    app.get("/api/v1/admin/users/:userId/records", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        ...
        const result = await db.query(`SELECT id, record_type, book_data, client_updated_at, updated_at, revision, deleted_at
       ...
       ORDER BY updated_at DESC LIMIT $5 OFFSET $6`, [request.params.userId, type, includeDeleted, q, limit, offset]);
        const count = await db.query(`SELECT count(*) FROM library_records
       WHERE user_id=$1 AND ($2 = '' OR record_type=$2) AND ($3::boolean OR deleted_at IS NULL)
         AND ($4 = '' OR COALESCE(book_data->>'title','') ILIKE '%' || $4 || '%' OR COALESCE(book_data->>'author','') ILIKE '%' || $4 || '%' OR COALESCE(book_data->>'name','') ILIKE '%' || $4 || '%')`, [request.params.userId, type, includeDeleted, q]);
    ...
    app.put("/api/v1/admin/users/:userId/records/:recordId", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        const recordType = request.body?.recordType;
        const data = request.body?.data;
        if (!recordType || !["book", "shelf", "goal"].includes(recordType))
            return reply.code(400).send({ error: "recordType must be book, shelf, or goal." });
        if (!data || typeof data !== "object" || Array.isArray(data))
            return reply.code(400).send({ error: "Record data must be a JSON object." });
        const dataId = data.id;
        if (dataId !== request.params.recordId)
            return reply.code(400).send({ error: "The JSON record id must match the record being edited." });
        const revision = await withAuditTransaction(db, request, "LIBRARY_RECORD_UPDATE", request.params.userId, request.params.recordId, { recordType }, async (client) => {
            const result = await client.query(`UPDATE library_records SET record_type=$1, book_data=$2::jsonb, client_updated_at=now(), deleted_at=NULL, revision=revision+1, updated_at=now()
    ...
    app.delete("/api/v1/admin/users/:userId/records/:recordId", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        if (request.body?.confirmation !== "DELETE RECORD")
            return reply.code(400).send({ error: "Confirmation must exactly match: DELETE RECORD" });
        const revision = await withAuditTransaction(db, request, "LIBRARY_RECORD_DELETE", request.params.userId, request.params.recordId, {}, async (client) => {
            const result = await client.query(`UPDATE library_records SET book_data=NULL, client_updated_at=now(), deleted_at=now(), revision=revision+1, updated_at=now()
    ...
    app.get("/api/v1/admin/audit", { preHandler: authenticateAdmin }, async (request, reply) => {
        const db = requirePool(reply);
        if (!db)
            return;
        const limit = Math.min(200, Math.max(1, Number(request.query.limit ?? 100) || 100));
        const offset = Math.max(0, Number(request.query.offset ?? 0) || 0);
        const q = request.query.q?.trim() ?? "";
        const result = await db.query(`SELECT a.id::text, a.admin_email, a.action, a.target_user_id, a.target_record_id, a.details, a.ip_address, a.created_at, u.email AS target_email
