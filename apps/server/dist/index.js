...
for (const candidate of [process.env.BOOKSTATS_ENV_FILE, ".env", "../../.env"].filter(Boolean)) {
    try {
        process.loadEnvFile?.(candidate);
        break;
    }
    catch { /* try the next optional location */ }
}
const app = Fastify({ logger: true, bodyLimit: 25 * 1024 * 1024, trustProxy: "127.0.0.1" });
const configuredOrigins = process.env.BOOKSTATS_CORS_ORIGIN?.split(",").map((item) => item.trim()).filter(Boolean);
...
app.addHook("onRequest", async (request, reply) => {
    ...
    if (request.method === "GET" && /^\/api\/v1\/covers\/[^/]+\/[^/]+$/.test(path))
        return;
...
function expandWebOriginAliases(origins) {
    ...
    for (const origin of origins) {
        try {
            ...
            if (url.protocol !== "https:" && url.protocol !== "http:")
                continue;
            if (url.hostname === "localhost" || url.hostname.endsWith(".localhost") || /^[0-9.]+$/.test(url.hostname))
                continue;
...
app.get("/api/v1/covers/:assetId/:accessToken", async (request, reply) => {
...
app.post("/api/v1/covers/archive", { preHandler: authenticate }, async (request, reply) => {
    const db = requireDb(reply);
    if (!db || !request.bookstatsUser)
        return;
    if (!request.bookstatsUser.emailVerified)
        return reply.code(403).send({ error: "Verify your email address before storing cloud cover assets." });
    const coverUrl = request.body?.coverUrl?.trim();
    if (!coverUrl)
        return reply.code(400).send({ error: "A selected cover is required." });
    try {
...
app.post("/api/v1/feedback", async (request, reply) => {
    if (!feedbackConfigured())
        return reply.code(503).send({ error: "Feedback delivery is not configured on this BookStats server." });
    const kind = request.body?.kind === "bug" ? "bug" : request.body?.kind === "feature" ? "feature" : undefined;
    const message = request.body?.message?.trim();
    if (!kind || !message)
        return reply.code(400).send({ error: "Choose a feedback type and enter a message." });
    if (message.length > 6000)
        return reply.code(400).send({ error: "Feedback messages can be up to 6,000 characters." });
    const contactEmail = request.body?.contactEmail?.trim();
    if (contactEmail && !normalizeEmail(contactEmail))
        return reply.code(400).send({ error: "Enter a valid contact email or leave it blank." });
    const now = Date.now();
    const cutoff = now - 15 * 60 * 1000;
    const attempts = (feedbackWindows.get(request.ip) ?? []).filter((timestamp) => timestamp > cutoff);
    if (attempts.length >= 5)
        return reply.code(429).send({ error: "Too many feedback messages were sent recently. Please try again later." });
    attempts.push(now);
    feedbackWindows.set(request.ip, attempts);
    const rawDiagnostics = request.body?.diagnostics ?? {};
    const allowedKeys = ["version", "platform", "storage", "signedIn", "emailVerified", "bookCount", "shelfCount"];
    const diagnostics = {};
    for (const key of allowedKeys) {
        const value = rawDiagnostics[key];
        if (typeof value === "string")
            diagnostics[key] = value.slice(0, 160);
...
app.post("/api/v1/auth/register", async (request, reply) => {
    ...
    try {
        ...
        const result = await client.query(`INSERT INTO users (email, display_name, password_hash) VALUES ($1, $2, $3)
       RETURNING id, email, display_name, created_at, email_verified_at`, [email, displayName, passwordHash]);
        ...
        if (emailConfigured()) {
            try {
                ...
                request.log.error(error, "Could not send verification email");
            }
        }
        ...
        if (isUniqueViolation(error))
            return reply.code(409).send({ error: "An account already exists for that email address." });
...
app.post("/api/v1/auth/login", async (request, reply) => {
    ...
    const client = await db.connect();
    try {
        const result = await client.query("SELECT id, email, display_name, created_at, email_verified_at, password_hash, disabled_at FROM users WHERE email = $1", [email]);
        const row = result.rows[0];
        if (!row || !(await verifyPassword(password, row.password_hash)))
            return reply.code(401).send({ error: "Incorrect email or password." });
        if (row.disabled_at)
            return reply.code(403).send({ error: "This BookStats account has been disabled. Contact the administrator for assistance." });
...
app.post("/api/v1/auth/logout", { preHandler: authenticate }, async (request, reply) => {
    ...
    if (!db)
        return;
    if (request.bookstatsTokenHash)
        await db.query("DELETE FROM sessions WHERE token_hash = $1", [request.bookstatsTokenHash]);
    return { ok: true };
});
app.post("/api/v1/auth/change-password", { preHandler: authenticate }, async (request, reply) => {
    ...
    if (!currentPassword || passwordError)
        return reply.code(400).send({ error: passwordError ?? "Current password is required." });
    const result = await db.query("SELECT password_hash FROM users WHERE id = $1", [request.bookstatsUser.id]);
    ...
    if (request.bookstatsTokenHash)
        await db.query("DELETE FROM sessions WHERE user_id = $1 AND token_hash <> $2", [request.bookstatsUser.id, request.bookstatsTokenHash]);
    if (emailConfigured())
        void sendPasswordChangedEmail(request.bookstatsUser.email, request.bookstatsUser.displayName).catch((error) => request.log.error(error, "Could not send password changed email"));
    return { ok: true };
});
app.post("/api/v1/auth/change-email", { preHandler: authenticate }, async (request, reply) => {
    const db = requireDb(reply);
    if (!db || !request.bookstatsUser)
        return;
    const currentPassword = request.body?.currentPassword ?? "";
    ...
    if (!currentPassword || !email)
        return reply.code(400).send({ error: "Current password and a valid new email are required." });
    const client = await db.connect();
    try {
        const result = await client.query("SELECT password_hash FROM users WHERE id = $1", [request.bookstatsUser.id]);
        if (!result.rows[0] || !(await verifyPassword(currentPassword, result.rows[0].password_hash)))
            return reply.code(401).send({ error: "Current password is incorrect." });
        await client.query("BEGIN");
        const changed = await client.query(`UPDATE users SET email = $1, email_verified_at = NULL, updated_at = now() WHERE id = $2 RETURNING id, email, display_name, created_at, email_verified_at`, [email, request.bookstatsUser.id]);
        let verificationSent = false;
        if (emailConfigured()) {
            const token = await issueToken(client, "email_verification_tokens", request.bookstatsUser.id, VERIFICATION_HOURS);
            try {
                await sendVerificationEmail(email, request.bookstatsUser.displayName, token);
                verificationSent = true;
            }
            catch (error) {
                request.log.error(error, "Could not send email-change verification message");
            }
        }
        ...
        if (isUniqueViolation(error))
            return reply.code(409).send({ error: "An account already exists for that email address." });
        request.log.error(error);
        return reply.code(500).send({ error: "Could not change the email address." });
    }
    finally {
        client.release();
    }
...
app.post("/api/v1/account/delete", { preHandler: authenticate }, async (request, reply) => {
    const db = requireDb(reply);
    if (!db || !request.bookstatsUser)
        return;
    const password = request.body?.password ?? "";
...
app.post("/api/v1/auth/resend-verification", { preHandler: authenticate }, async (request, reply) => {
    ...
    if (!emailConfigured())
        return reply.code(503).send({ error: "Email delivery is not configured on this BookStats server." });
    const client = await db.connect();
    try {
...
app.post("/api/v1/auth/verify-email", async (request, reply) => {
    ...
    try {
        await client.query("BEGIN");
        const result = await client.query(`SELECT u.id, u.email, u.display_name, u.created_at, u.email_verified_at, t.id AS token_id
       ...
       FOR UPDATE OF t`, [sha256(token)]);
        ...
        const refreshed = await client.query("SELECT id, email, display_name, created_at, email_verified_at FROM users WHERE id = $1", [row.id]);
        await client.query("COMMIT");
...
app.post("/api/v1/auth/forgot-password", async (request, reply) => {
    ...
    const client = await db.connect();
    try {
        const result = await client.query("SELECT id, email, display_name, created_at, email_verified_at FROM users WHERE email = $1", [email]);
        const user = result.rows[0] ? accountFromRow(result.rows[0]) : undefined;
        if (!user || !emailConfigured())
            return generic;
        if (await hasRecentToken(client, "password_reset_tokens", user.id))
            return generic;
...
app.post("/api/v1/auth/reset-password", async (request, reply) => {
    ...
    if (!token || passwordError)
        return reply.code(400).send({ error: passwordError ?? "Reset token is required." });
    const client = await db.connect();
    let changedUser;
    try {
        await client.query("BEGIN");
...
app.post("/api/v1/sync", { preHandler: authenticate }, async (request, reply) => {
    ...
    if (!db || !request.bookstatsUser)
        return;
    if (!request.bookstatsUser.emailVerified)
        return reply.code(403).send({ error: "Verify your email address before enabling cloud synchronization." });
    ...
    const validCoverAssets = new Set();
    if (assetIds.length) {
        const ownedAssets = await db.query("SELECT id, access_token FROM cover_assets WHERE user_id = $1 AND id::text = ANY($2::text[])", [request.bookstatsUser.id, assetIds]);
        for (const row of ownedAssets.rows)
            validCoverAssets.add(`${row.id}:${row.access_token}`);
    }
    const changes = [];
    for (const change of rawChanges) {
        if (change?.entityType === "shelf" || change?.entityType === "goal" || change?.deleted || !change?.book) {
            changes.push(change);
            continue;
        }
    ...
    try {
        ...
        for (const change of changes) {
            if (!change?.id || !change.clientUpdatedAt)
                continue;
            ...
            if (!change.deleted && (!payload || payload.id !== change.id))
                continue;
            const incomingTime = new Date(change.clientUpdatedAt);
            if (Number.isNaN(incomingTime.getTime()))
                continue;
            const existing = await client.query("SELECT client_updated_at FROM library_records WHERE user_id = $1 AND id = $2 FOR UPDATE", [request.bookstatsUser.id, change.id]);
            if (existing.rows[0] && existing.rows[0].client_updated_at.getTime() > incomingTime.getTime())
                continue;
            await client.query(`INSERT INTO library_records (id, user_id, record_type, book_data, client_updated_at, deleted_at)
         ...
         ON CONFLICT (user_id, id) DO UPDATE SET
           ...
           updated_at = now()`, [change.id, request.bookstatsUser.id, entityType, change.deleted ? null : JSON.stringify(payload ?? null), incomingTime.toISOString(), change.deleted ? incomingTime.toISOString() : null]);
            accepted += 1;
        }
        const serverCursorResult = await client.query("SELECT now() AS cursor");
        const serverCursor = serverCursorResult.rows[0].cursor.toISOString();
        const pulled = await client.query(`SELECT id, record_type, book_data, client_updated_at, updated_at, revision, deleted_at
       FROM library_records WHERE user_id = $1 AND updated_at > $2 ORDER BY updated_at ASC`, [request.bookstatsUser.id, cursor]);
        await client.query("COMMIT");
        const records = pulled.rows.map((row) => {
            const entityType = row.record_type === "shelf" ? "shelf" : row.record_type === "goal" ? "goal" : "book";
            return {
                id: row.id,
                entityType,
                deleted: Boolean(row.deleted_at),
                book: entityType === "book" ? row.book_data ?? undefined : undefined,
                shelf: entityType === "shelf" ? row.book_data ?? undefined : undefined,
                goal: entityType === "goal" ? row.book_data ?? undefined : undefined,
...
app.get("/api/v1/metadata/search", async (request, reply) => {
    const q = request.query.q?.trim();
    const isbn = request.query.isbn?.replace(/[^0-9Xx]/g, "").toUpperCase();
    if (!q && !isbn)
        return reply.code(400).send({ error: "Provide q or isbn." });
    ...
    if (cached)
        return { results: cached, cached: true, providers: metadataProviderStatuses() };
    try {
...
app.post("/api/v1/metadata/details", async (request, reply) => {
    const candidate = request.body?.candidate;
    if (!candidate?.workId || !candidate.title)
        return reply.code(400).send({ error: "A metadata candidate is required." });
...
async function prepareBookCoverForCloud(db, userId, book, validCoverAssets, log) {
    const selected = book.coverUrl?.trim();
    if (!book.coverAssetId && !book.coverArchivePending && !selected?.startsWith("data:"))
        return book;
    const next = { ...book };
    ...
    if (!hasOwnedAsset) {
        ...
        if (!selected && next.coverSourceUrl && /^https?:\/\//i.test(next.coverSourceUrl))
            next.coverUrl = next.coverSourceUrl;
    }
...
async function authenticate(request, reply) {
    ...
    const result = await db.query(`SELECT u.id, u.email, u.display_name, u.created_at, u.email_verified_at
...
async function createSession(client, userId) {
    ...
    await client.query(`INSERT INTO sessions (user_id, token_hash, expires_at) VALUES ($1, $2, now() + ($3 || ' days')::interval)`, [userId, sha256(token), SESSION_DAYS]);
    return token;
}
async function issueToken(client, table, userId, expiresHours) {
    const token = randomBytes(32).toString("base64url");
    await client.query(`UPDATE ${table} SET used_at = now() WHERE user_id = $1 AND used_at IS NULL`, [userId]);
    await client.query(`INSERT INTO ${table} (user_id, token_hash, expires_at) VALUES ($1, $2, now() + ($3 || ' hours')::interval)`, [userId, sha256(token), expiresHours]);
    return token;
}
async function hasRecentToken(client, table, userId) {
    const result = await client.query(`SELECT 1 FROM ${table} WHERE user_id = $1 AND used_at IS NULL AND created_at > now() - ($2 || ' minutes')::interval LIMIT 1`, [userId, TOKEN_THROTTLE_MINUTES]);
    return Boolean(result.rows[0]);
}
function sha256(value) { return createHash("sha256").update(value).digest("hex"); }
function isUniqueViolation(error) { return Boolean(error && typeof error === "object" && "code" in error && error.code === "23505"); }
...
async function getCached(key) {
    if (!pool)
        return undefined;
    try {
        const result = await pool.query("SELECT payload FROM metadata_cache WHERE cache_key = $1 AND expires_at > now()", [key]);
...
async function putCached(key, payload, ttlSeconds) {
    if (!pool)
        return;
    try {
        await pool.query(`INSERT INTO metadata_cache (cache_key, payload, expires_at) VALUES ($1, $2::jsonb, now() + ($3 || ' seconds')::interval)
       ON CONFLICT (cache_key) DO UPDATE SET payload = EXCLUDED.payload, expires_at = EXCLUDED.expires_at, updated_at = now()`, [key, JSON.stringify(payload), ttlSeconds]);
    }
