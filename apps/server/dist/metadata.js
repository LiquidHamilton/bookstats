...
const metadataUserAgent = () => process.env.BOOKSTATS_METADATA_USER_AGENT ?? "BookStats/1.0.4 (local-development)";
...
export function metadataProviderStatuses() {
    return [
...
export function isbn13From10(value) {
...
export function isbn10From13(value) {
    const isbn = normalizeIsbn(value);
    if (!isbn || isbn.length !== 13 || !/^978\d{10}$/.test(isbn))
        return undefined;
...
export function isbnEquivalent(left, right) {
    const a = normalizeIsbn(left);
    const b = normalizeIsbn(right);
    if (!a || !b)
        return false;
    if (a === b)
        return true;
...
function preferredIsbn(values, requested) {
    const clean = [...new Set(values.map(normalizeIsbn).filter((value) => Boolean(value)))];
    const wanted = normalizeIsbn(requested);
    if (wanted) {
        const exact = clean.find((value) => isbnEquivalent(value, wanted));
        if (exact)
            return exact.length === 13 ? exact : isbn13From10(exact) ?? exact;
    }
    return clean.find((value) => value.length === 13) ?? clean[0];
}
export async function searchAllMetadata(query, isbn) {
    const requestedIsbn = normalizeIsbn(isbn);
    const tasks = [
        requestedIsbn ? openLibraryByIsbn(requestedIsbn) : openLibrarySearch(query)
    ];
    if (googleKey())
        tasks.push(requestedIsbn ? googleByIsbn(requestedIsbn) : googleSearch(query));
    if (hardcoverToken())
        tasks.push(requestedIsbn ? hardcoverByIsbn(requestedIsbn) : hardcoverSearch(query));
...
export async function getMetadataDetails(candidate) {
    ...
    if (merged.series && merged.seriesVolume && merged.seriesMetadata)
        return merged;
...
async function detailsForRef(ref, isbn) {
    if (ref.provider === "openlibrary")
        return openLibraryDetails(ref, isbn);
    if (ref.provider === "googlebooks")
        return googleDetails(ref);
    return hardcoverDetails(ref);
}
function mergeSearchCandidates(candidates, query) {
...
const editionPriority = ["googlebooks", "hardcover", "openlibrary", "aggregate"];
const workPriority = ["hardcover", "googlebooks", "openlibrary", "aggregate"];
function dedupeSeriesMemberships(memberships) {
...
function candidateSeriesMemberships(candidate) {
...
function preferredSeriesMembership(candidate) {
...
function sameSeriesName(left, right) {
    return Boolean(left && right && normalizeText(left) === normalizeText(right));
}
export function mergeCandidates(candidates, requestedIsbn, exactEdition = false) {
    const nonEmpty = candidates.filter((candidate) => candidate?.title);
    if (!nonEmpty.length)
        throw new Error("No metadata candidates were available to merge.");
    const sortedEdition = sortProviders(nonEmpty, editionPriority);
    const sortedWork = sortProviders(nonEmpty, workPriority);
    const fieldSources = {};
    const choose = (field, list = sortedEdition) => {
        const found = list.find((candidate) => hasValue(candidate[field]));
        if (found && metadataFieldNames.has(field))
            fieldSources[field] = found.source === "aggregate" ? inferFieldProvider(found, field) : found.source;
        return found?.[field];
    };
    ...
    const source = refs.length > 1 ? "aggregate" : refs[0]?.provider ?? primary.source;
    ...
    if (isbn)
        fieldSources.isbn = sortedEdition.find((candidate) => candidate.isbn && isbnEquivalent(candidate.isbn, isbn))?.source ?? source;
    if (covers[0])
        fieldSources.coverUrl = nonEmpty.find((candidate) => candidate.coverUrl === covers[0] || candidate.coverUrls?.includes(covers[0]))?.source ?? source;
    return {
        source,
...
const metadataFieldNames = new Set(["title", "author", "additionalAuthors", "isbn", "series", "seriesVolume", "publicationYear", "publisher", "language", "pages", "format", "genre", "description", "coverUrl"]);
function inferFieldProvider(candidate, field) { return candidate.fieldSources?.[field] ?? candidate.source; }
function sortProviders(candidates, priority) { return [...candidates].sort((a, b) => priority.indexOf(a.source) - priority.indexOf(b.source)); }
function hasValue(value) { return value !== undefined && value !== null && value !== "" && (!Array.isArray(value) || value.length > 0); }
function uniqueStrings(values) { return [...new Set(values.map((value) => value?.trim()).filter((value) => Boolean(value)))]; }
function dedupeRefs(refs) { const seen = new Set(); return refs.filter((ref) => { const key = `${ref.provider}:${ref.workId}:${ref.editionId ?? ""}`; if (seen.has(key))
    return false; seen.add(key); return true; }); }
function normalizeText(value) { return value?.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLocaleLowerCase().replace(/[^a-z0-9]+/g, " ").trim() ?? ""; }
function searchRelevanceScore(candidate, query) {
    ...
    const authorTokens = author.split(" ").filter(Boolean);
    const tokenCoverage = (needles, haystack) => needles.length ? needles.filter((token) => haystack.includes(token)).length / needles.length : 0;
    let score = 0;
    if (title === wanted)
        score += 1000;
    else {
    ...
    if (bundleTerms.some((term) => title.includes(term)) && !bundleTerms.some((term) => wanted.includes(term)))
        score -= 375;
...
async function seriesEnrichmentFor(candidate) {
    const query = `${candidate.title} ${candidate.author}`.trim();
    let prospects = [];
    if (hardcoverToken())
        prospects = await hardcoverSearch(query).catch(() => []);
    if (!prospects.some((item) => item.series || item.seriesVolume || item.seriesMetadata))
        prospects = await openLibrarySearch(query).catch(() => []);
    ...
    if (best.source === "hardcover") {
        ...
        if (ref)
            return await hardcoverDetails(ref).catch(() => best);
    }
    return best;
}
function workMatchScore(target, candidate) {
    const leftTitle = normalizeWorkTitle(target.title);
    const rightTitle = normalizeWorkTitle(candidate.title);
    const leftAuthor = normalizeText(target.author);
    const rightAuthor = normalizeText(candidate.author);
    const leftTitleTokens = leftTitle.split(" ").filter(Boolean);
    const rightTitleTokens = rightTitle.split(" ").filter(Boolean);
    const coverage = (needles, haystack) => needles.length ? needles.filter((token) => haystack.includes(token)).length / needles.length : 0;
    ...
    const leftAuthorTokens = leftAuthor.split(" ").filter(Boolean);
    const rightAuthorTokens = rightAuthor.split(" ").filter(Boolean);
    const authorOverlap = Math.max(coverage(leftAuthorTokens, rightAuthorTokens), coverage(rightAuthorTokens, leftAuthorTokens));
    const usefulAuthor = (value) => Boolean(value && value !== "unknown author" && value !== "unknown");
    if (usefulAuthor(leftAuthor) && usefulAuthor(rightAuthor) && authorOverlap < 0.5)
        return 0;
    if (leftAuthor && rightAuthor)
        score += authorOverlap * 250;
    return score;
}
...
function applySeriesEnrichment(target, enrichment) {
...
function seriesNamesCompatible(left, right) {
    const normalize = (value) => normalizeText(value).replace(/^the\s+/, "");
    const a = normalize(left);
    const b = normalize(right);
    return a === b || a.includes(b) || b.includes(a);
}
function yearFromDate(value) { const match = value?.match(/\b(1[4-9]\d{2}|20\d{2}|21\d{2})\b/); return match ? Number(match[1]) : undefined; }
function safeNumber(value) { const number = Number(value); return Number.isFinite(number) && number > 0 ? number : undefined; }
function sourceRef(provider, workId, editionId, exactIsbn, sourceUrl) { return { provider, workId, editionId, exactIsbn, sourceUrl }; }
function googleSeriesPosition(entry) {
...
async function googleSearch(query) {
    ...
    return (data.items ?? []).map((volume) => googleCandidate(volume, false)).filter((candidate) => Boolean(candidate));
}
async function googleByIsbn(isbn) {
    ...
    appendGoogleKey(url);
    const data = await jsonFetch(url.toString());
    return (data.items ?? []).filter((volume) => googleIdentifiers(volume).some((value) => isbnEquivalent(value, isbn))).map((volume) => googleCandidate(volume, true)).filter((candidate) => Boolean(candidate));
}
async function googleDetails(ref) {
    const id = ref.editionId ?? ref.workId;
    const url = new URL(`${GOOGLE_BOOKS_API}/volumes/${encodeURIComponent(id)}`);
    url.searchParams.set("projection", "full");
    url.searchParams.set("includeNonComicsSeries", "true");
    appendGoogleKey(url);
    const volume = await jsonFetch(url.toString());
...
function googleCandidate(volume, exactEdition) {
    ...
    if (seriesEntry) {
        fieldSources.series = "googlebooks";
        fieldSources.seriesVolume = "googlebooks";
    }
    return {
        source: "googlebooks",
        workId: volume.id,
...
function googleIdentifiers(volume) { return (volume.volumeInfo?.industryIdentifiers ?? []).map((entry) => normalizeIsbn(entry.identifier)).filter((value) => Boolean(value)); }
function appendGoogleKey(url) { const key = googleKey(); if (key)
    url.searchParams.set("key", key); }
function upgradeGoogleCover(value) { return value?.replace(/^http:/, "https:").replace(/&zoom=\d/, "&zoom=2"); }
async function googleSeriesMetadata(seriesId) {
    const seriesUrl = new URL(`${GOOGLE_BOOKS_API}/series/get`);
    seriesUrl.searchParams.set("series_id", seriesId);
    appendGoogleKey(seriesUrl);
    const membershipUrl = new URL(`${GOOGLE_BOOKS_API}/series/membership/get`);
    membershipUrl.searchParams.set("series_id", seriesId);
    membershipUrl.searchParams.set("page_size", "40");
    appendGoogleKey(membershipUrl);
    const [seriesData, membership] = await Promise.all([jsonFetch(seriesUrl.toString()), jsonFetch(membershipUrl.toString())]);
    const series = seriesData.series?.[0];
    const books = (membership.member ?? []).map((volume) => {
        const info = volume.volumeInfo;
        const item = info?.seriesInfo?.volumeSeries?.find((entry) => entry.seriesId === seriesId) ?? info?.seriesInfo?.volumeSeries?.[0];
    ...
    if (!series?.title && books.length === 0)
        return undefined;
...
async function hardcoverByIsbn(isbn) {
    ...
    const data = await hardcoverGraphql(query, { isbn13, isbn10 });
...
async function hardcoverSearch(queryText) {
    const searchQuery = `query Search($query: String!) { search(query: $query, per_page: 12) { ids results } }`;
    const searched = await hardcoverGraphql(searchQuery, { query: queryText });
    const ids = (searched.search?.ids ?? []).filter((id) => Number.isInteger(id)).slice(0, 12);
    if (!ids.length)
        return [];
    ...
    const data = await hardcoverGraphql(query, { ids });
...
async function hardcoverDetails(ref) {
    const editionId = Number(ref.editionId);
    const bookId = Number(ref.workId);
    let candidate;
    if (Number.isInteger(editionId) && editionId > 0) {
      ...
      book { id title subtitle description pages release_year image { url } contributions { contribution author { name } }
        ...
        const data = await hardcoverGraphql(query, { id: editionId });
        if (data.editions_by_pk)
            candidate = hardcoverEditionCandidate(data.editions_by_pk, Boolean(ref.exactIsbn));
    }
      ...
      book_series { position featured compilation series { id name books_count primary_books_count is_completed } }
        ...
        const data = await hardcoverGraphql(query, { id: bookId });
        if (data.books_by_pk)
            candidate = hardcoverBookCandidate(data.books_by_pk);
    }
    if (!candidate)
        return undefined;
    const series = chooseHardcoverSeriesFromCandidate(candidate);
    if (series)
        candidate.seriesMetadata = await hardcoverSeriesMetadata(series.id).catch(() => candidate?.seriesMetadata);
    if (candidate.seriesMetadata) {
        candidate.series = candidate.seriesMetadata.name;
        candidate.seriesMemberships = dedupeSeriesMemberships((candidate.seriesMemberships ?? []).map((membership) => membership.provider === "hardcover" && membership.seriesId === candidate.seriesMetadata?.id
            ? { ...membership, name: candidate.seriesMetadata.name, metadata: candidate.seriesMetadata }
            : membership));
    }
    return candidate;
}
function hardcoverEditionCandidate(edition, exactEdition) {
...
function hardcoverBookCandidate(book) {
    if (!book.title)
        return undefined;
    const edition = book.editions?.[0];
    if (edition) {
        edition.book = book;
        return hardcoverEditionCandidate(edition, false);
    }
    const authors = hardcoverAuthors(book.contributions);
    const seriesLink = chooseHardcoverSeries(book);
    const cover = book.image?.url ?? undefined;
    const seriesMemberships = hardcoverSeriesMemberships(book);
...
function hardcoverAuthors(contributions) { const named = uniqueStrings((contributions ?? []).map((item) => item.author?.name)); const authorRole = uniqueStrings((contributions ?? []).filter((item) => !item.contribution || /author|writer/i.test(item.contribution)).map((item) => item.author?.name)); return authorRole.length ? authorRole : named; }
function hardcoverSeriesMetadataStub(series) {
...
function rankedHardcoverSeries(book) {
    const entries = [book.featured_book_series, ...(book.book_series ?? [])]
        .filter((entry) => Boolean(entry?.series && !entry.compilation));
    const unique = [...new Map(entries.map((entry) => [entry.series.id, entry])).values()];
    return unique.sort((a, b) => {
        ...
        if (aFinite !== bFinite)
            return aFinite ? -1 : 1;
        if (aFinite && bFinite && aCount !== bCount)
...
function hardcoverSeriesMemberships(book) {
...
function chooseHardcoverSeries(book) {
    const ranked = rankedHardcoverSeries(book);
    return ranked[0] ?? book.featured_book_series ?? book.book_series?.[0];
}
function chooseHardcoverSeriesFromCandidate(candidate) { const id = candidate.seriesMetadata?.provider === "hardcover" ? Number(candidate.seriesMetadata.id) : NaN; return Number.isInteger(id) ? { id } : undefined; }
async function hardcoverSeriesMetadata(seriesId) {
    ...
    const data = await hardcoverGraphql(query, { id: seriesId });
...
async function hardcoverGraphql(query, variables) {
    ...
    if (!token)
        throw new Error("Hardcover is not configured.");
    const wait = Math.max(0, 1100 - (Date.now() - lastHardcoverRequest));
    if (wait)
        await new Promise((resolve) => setTimeout(resolve, wait));
    lastHardcoverRequest = Date.now();
    ...
    if (!response.ok || payload.errors?.length || !payload.data)
        throw new Error(`Hardcover request failed (${response.status}): ${payload.errors?.map((error) => error.message).filter(Boolean).join("; ") || response.statusText}`);
    return payload.data;
}
async function openLibrarySearch(query) {
    ...
    url.searchParams.set("fields", "key,title,author_name,first_publish_year,isbn,cover_i,edition_key,number_of_pages_median,publisher,language,subject,series,editions,editions.key,editions.title,editions.isbn,editions.cover_i,editions.publish_date,editions.publish_year,editions.publisher,editions.language,editions.number_of_pages,editions.physical_format,editions.series");
    const data = await openLibraryFetch(url.toString());
    return data.docs.map(candidateFromOpenLibrarySearch).filter((candidate) => Boolean(candidate));
}
async function openLibraryByIsbn(isbn) {
    try {
        const edition = await openLibraryFetch(`${OPEN_LIBRARY_API}/isbn/${encodeURIComponent(isbn)}.json`);
        ...
        url.searchParams.set("fields", "key,title,author_name,first_publish_year,isbn,cover_i,edition_key,number_of_pages_median,publisher,language,subject,series");
        const data = await openLibraryFetch(url.toString());
        ...
        return data.docs.map(candidateFromOpenLibrarySearch)
            .filter((candidate) => Boolean(candidate?.isbn && isbnEquivalent(candidate.isbn, isbn)))
            .map((candidate) => ({
            ...candidate,
            editionId: undefined,
            exactEdition: true,
...
async function openLibraryDetails(ref, isbn) {
    const editionId = sanitizeOlid(ref.editionId, "M");
    const workId = sanitizeOlid(ref.workId, "W");
    if (editionId) {
        const edition = await openLibraryFetch(`${OPEN_LIBRARY_API}/books/${editionId}.json`);
        return openLibraryCandidateFromEdition(edition, { ...ref, workId: workId ?? sanitizeOlid(edition.works?.[0]?.key, "W") ?? "", editionId }, ref.exactIsbn ?? isbn);
    }
    if (!workId)
        return undefined;
    const work = await openLibraryFetch(`${OPEN_LIBRARY_API}/works/${workId}.json`);
    const authors = await openLibraryAuthorNames(work);
    const seriesMemberships = openLibrarySeriesMemberships(work.series);
    const seriesInfo = seriesMemberships[0];
...
async function openLibraryCandidateFromEdition(edition, ref, requestedIsbn) {
    const workId = sanitizeOlid(ref.workId, "W") ?? sanitizeOlid(edition.works?.[0]?.key, "W");
    const editionId = sanitizeOlid(edition.key, "M") ?? sanitizeOlid(ref.editionId, "M");
    const work = workId ? await openLibraryFetch(`${OPEN_LIBRARY_API}/works/${workId}.json`).catch(() => undefined) : undefined;
    ...
    const isbn = preferredIsbn([...(edition.isbn_13 ?? []), ...(edition.isbn_10 ?? [])], requestedIsbn);
    const seriesMemberships = openLibrarySeriesMemberships([...(edition.series ?? []), ...(work?.series ?? [])]);
    const seriesInfo = seriesMemberships[0];
    const covers = uniqueStrings((edition.covers ?? []).filter((id) => id > 0).map(coverUrl));
    const exact = Boolean(requestedIsbn && isbn && isbnEquivalent(isbn, requestedIsbn));
...
function candidateFromOpenLibrarySearch(doc) {
    const workId = sanitizeOlid(doc.key, "W");
    if (!workId || !doc.title)
        return undefined;
    const matchedEdition = doc.editions?.docs?.[0];
    ...
    const seriesInfo = seriesMemberships[0] ?? (() => { const parsed = parseSeriesFromTitle(title); return parsed ? { provider: "openlibrary", name: parsed.name, volume: parsed.volume } : undefined; })();
    if (seriesInfo && !seriesMemberships.length)
        seriesMemberships.push(seriesInfo);
    const isbn = preferredIsbn(matchedEdition?.isbn?.length ? matchedEdition.isbn : doc.isbn ?? []);
    const coverId = matchedEdition?.cover_i ?? doc.cover_i;
    const cover = coverId ? coverUrl(coverId) : undefined;
    const publicationYear = matchedEdition?.publish_year?.[0] ?? yearFromDate(matchedEdition?.publish_date?.[0]) ?? doc.first_publish_year;
    const publisher = matchedEdition?.publisher?.[0] ?? doc.publisher?.[0];
    const language = matchedEdition?.language?.[0] ?? doc.language?.[0];
    const pages = matchedEdition?.number_of_pages ?? doc.number_of_pages_median;
    const format = mapBookFormat(matchedEdition?.physical_format);
...
async function openLibraryAuthorNames(work) { return uniqueStrings(await Promise.all((work.authors ?? []).map((entry) => openLibraryAuthorName(entry.author?.key)))); }
async function openLibraryAuthorName(key) { if (!key)
    return undefined; try {
    return (await openLibraryFetch(`${OPEN_LIBRARY_API}${key}.json`)).name;
}
catch {
    return undefined;
} }
function languageFromOpenLibrary(key) { return key?.split("/").filter(Boolean).at(-1); }
function coverUrl(id) { return `https://covers.openlibrary.org/b/id/${id}-L.jpg`; }
async function openLibraryFetch(url) { return jsonFetch(url, { "User-Agent": metadataUserAgent(), Accept: "application/json" }); }
function sanitizeOlid(value, suffix) { if (!value)
    return undefined; const id = value.replace(/^\/(works|books)\//, ""); return new RegExp(`^OL\\d+${suffix}$`).test(id) ? id : undefined; }
function openLibrarySeriesMemberships(values) {
    return dedupeSeriesMemberships((values ?? []).flatMap((value) => {
        const parsed = parseSeriesLabel(value);
        return parsed ? [{ provider: "openlibrary", name: parsed.name, volume: parsed.volume }] : [];
    }));
}
function parseSeriesLabel(value) {
    if (!value?.trim())
        return undefined;
    ...
    for (const pattern of patterns) {
        ...
        if (match)
            return { name: match[1].trim(), volume: match[2] };
    }
    return { name: clean };
}
...
function providerFieldSources(provider, fields) { return Object.fromEntries(fields.map((field) => [field, provider])); }
function mapBookFormat(value) { if (!value)
    return undefined; const text = value.toLocaleLowerCase(); if (text.includes("mass market"))
    return "Mass Market Paperback"; if (text.includes("paperback") || text.includes("softcover"))
    return "Paperback"; if (text.includes("hardcover") || text.includes("hardback"))
    return "Hardcover"; if (text.includes("ebook") || text.includes("kindle") || text.includes("digital"))
    return "eBook"; if (text.includes("audio"))
    return "Audiobook"; if (text.includes("graphic"))
    return "Graphic Novel"; if (text.includes("omnibus"))
    return "Omnibus"; return "Other"; }
function compactPrimarySeriesBooks(books, expectedPrimary) {
    const unique = [...new Map(books.map((book) => [book.providerId, book])).values()];
    ...
    if (!expected || expected <= 0 || expected > 500)
        return unique;
    ...
    if (byPosition.size < Math.min(expected, needed))
        return unique;
...
function seriesBookSort(a, b) { const left = Number(a.position); const right = Number(b.position); if (Number.isFinite(left) && Number.isFinite(right))
    return left - right; if (Number.isFinite(left))
    return -1; if (Number.isFinite(right))
    return 1; return String(a.position ?? "").localeCompare(String(b.position ?? "")); }
async function jsonFetch(url, headers) { const response = await fetch(url, { headers }); if (!response.ok)
    throw new Error(`Metadata request failed (${response.status}) for ${new URL(url).hostname}`); return response.json(); }
