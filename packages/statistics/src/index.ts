import { normalizedReadDates, normalizedReadingSessions, type Book, type LibrarySummary, type ReadingGoal, type ReadingStatus, type SeriesCatalogBook, type SeriesCompletionOverride, type SeriesMetadata } from "@bookstats/domain";
...
function sortedDistribution(counts: Map<string, number>): DistributionDatum[] {
...
export function readEvents(books: Book[]): ReadEvent[] {
...
export function summarizeLibrary(books: Book[]): LibrarySummary {
...
export function statusDistribution(books: Book[]): DistributionDatum[] {
  ...
  for (const book of books) counts.set(book.status, (counts.get(book.status) ?? 0) + 1);
  return [...counts.entries()].map(([status, value]) => ({ name: labels[status], value }));
}

export function ratingDistribution(books: Book[]): DistributionDatum[] {
  const buckets = Array.from({ length: 10 }, (_, index) => ({ name: `${(index + 1) / 2}★`, value: 0 }));
  for (const book of books) {
    if (typeof book.rating !== "number" || book.rating <= 0) continue;
...
export function formatDistribution(books: Book[]): DistributionDatum[] {
...
export function genreDistribution(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const book of books) {
...
export function seriesDistribution(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const book of books) {
...
export function authorLibraryDistribution(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const book of books) {
    const authors = [book.author, ...book.additionalAuthors].map((author) => author.trim()).filter(Boolean);
    for (const author of new Set(authors)) counts.set(author, (counts.get(author) ?? 0) + 1);
  }
  return sortedDistribution(counts);
}

export function authorReadDistribution(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const book of books) {
    const datedReads = normalizedReadDates(book).length;
    const readings = Math.max(datedReads, book.status === "read" ? 1 : 0);
...
export function booksReadByYear(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const event of readEvents(books)) counts.set(String(event.year), (counts.get(String(event.year)) ?? 0) + 1);
  return [...counts.entries()].map(([name, value]) => ({ name, value })).sort((a, b) => a.name.localeCompare(b.name));
}

export function pagesReadByYear(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const event of readEvents(books)) counts.set(String(event.year), (counts.get(String(event.year)) ?? 0) + event.pages);
  return [...counts.entries()].map(([name, value]) => ({ name, value })).sort((a, b) => a.name.localeCompare(b.name));
}

export function booksAddedByYear(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const book of books) {
...
export function publicationDecadeDistribution(books: Book[]): DistributionDatum[] {
  const counts = new Map<string, number>();
  for (const book of books) {
    if (!book.publicationYear) continue;
...
export function pageLengthDistribution(books: Book[]): DistributionDatum[] {
  const buckets = [
...
export function averageRatingByAuthor(books: Book[]): Array<DistributionDatum & { count: number }> {
...
export function readingMonthDistribution(books: Book[]): DistributionDatum[] {
  const labels = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  const counts = Array(12).fill(0) as number[];
  for (const event of readEvents(books)) if (event.month >= 1 && event.month <= 12) counts[event.month - 1] += 1;
  return labels.map((name, index) => ({ name, value: counts[index] }));
}

export interface MonthlyReadingDatum { name: string; books: number; pages: number; }
export function monthlyReadingForYear(books: Book[], year: number): MonthlyReadingDatum[] {
  const labels = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  const rows = labels.map((name) => ({ name, books: 0, pages: 0 }));
  for (const event of readEvents(books)) {
    if (event.year !== year || event.month < 1 || event.month > 12) continue;
...
export function newAuthorsByYear(books: Book[]): DistributionDatum[] {
  const firstReadByAuthor = new Map<string, { name: string; date: string }>();
  for (const event of readEvents(books)) {
    const key = event.author.trim().toLocaleLowerCase();
    if (!key) continue;
    const existing = firstReadByAuthor.get(key);
    if (!existing || event.date < existing.date) firstReadByAuthor.set(key, { name: event.author.trim(), date: event.date });
  }
  const counts = new Map<string, number>();
  for (const { date } of firstReadByAuthor.values()) {
    const year = yearOf(date);
    if (!year) continue;
    const key = String(year); counts.set(key, (counts.get(key) ?? 0) + 1);
  }
  return [...counts.entries()].map(([name, value]) => ({ name, value })).sort((a, b) => a.name.localeCompare(b.name));
}

export function ratingTrendByYear(books: Book[]): DistributionDatum[] {
  const bookById = new Map(books.map((book) => [book.id, book]));
  const values = new Map<string, number[]>();
  for (const event of readEvents(books)) {
    const rating = bookById.get(event.bookId)?.rating;
    if (typeof rating !== "number") continue;
    const key = String(event.year); const list = values.get(key) ?? []; list.push(rating); values.set(key, list);
  }
...
export function readFormatDistribution(books: Book[]): DistributionDatum[] {
  const bookById = new Map(books.map((book) => [book.id, book]));
  const counts = new Map<string, number>();
  for (const event of readEvents(books)) {
...
export function readGenreDistribution(books: Book[]): DistributionDatum[] {
  const bookById = new Map(books.map((book) => [book.id, book]));
  const counts = new Map<string, number>();
  for (const event of readEvents(books)) {
...
export function readOwnershipDistribution(books: Book[]): DistributionDatum[] {
  const bookById = new Map(books.map((book) => [book.id, book]));
  let owned = 0; let unowned = 0;
  for (const event of readEvents(books)) {
    if (bookById.get(event.bookId)?.owned) owned += 1; else unowned += 1;
  }
...
export interface ReadingExtremes {
  longest?: { title: string; author: string; pages: number };
  shortest?: { title: string; author: string; pages: number };
}
export function readingExtremes(books: Book[]): ReadingExtremes {
...
export function readingGoalProgress(goal: ReadingGoal, books: Book[], today = new Date().toISOString().slice(0, 10)): GoalProgress {
  const events = readEvents(books);
  const inRange = events.filter((event) => event.date >= goal.startDate && event.date <= goal.endDate);
  let current = 0;
  switch (goal.metric) {
    case "books": current = inRange.length; break;
    case "pages": current = inRange.reduce((sum, event) => sum + event.pages, 0); break;
    case "rereads": current = inRange.filter((event) => event.isReread).length; break;
    case "owned_books": {
    ...
    case "new_authors": {
      const firstReadByAuthor = new Map<string, string>();
      for (const event of events) {
        const key = event.author.trim().toLocaleLowerCase();
        if (!key) continue;
        const existing = firstReadByAuthor.get(key);
        if (!existing || event.date < existing) firstReadByAuthor.set(key, event.date);
      }
  ...
  const start = dayNumber(goal.startDate); const end = dayNumber(goal.endDate); const now = dayNumber(today);
  let elapsedPercent = 0; let daysRemaining = 0; let projected: number | null = null;
  if (start !== undefined && end !== undefined && now !== undefined && end >= start) {
    ...
    if (elapsedDays > 0 && now <= end) projected = current / elapsedDays * totalDays;
  }
...
export function readingPaceSummary(books: Book[]): ReadingPaceSummary {
  const events = readEvents(books);
...
function normalizeSeriesIsbn(value?: string): string | undefined {
  const isbn = value?.replace(/[^0-9Xx]/g, "").toUpperCase();
  return isbn && (isbn.length === 10 || isbn.length === 13) ? isbn : undefined;
}

function seriesIsbn13From10(value: string): string | undefined {
...
function seriesIsbnEquivalent(left?: string, right?: string): boolean {
  const a = normalizeSeriesIsbn(left); const b = normalizeSeriesIsbn(right);
  if (!a || !b) return false;
  if (a === b) return true;
...
function chooseSeriesCatalog(items: Book[]): SeriesMetadata | undefined {
  const catalogs = items.map((book) => book.seriesMetadata).filter((value): value is SeriesMetadata => Boolean(value));
  const providerRank: Record<SeriesMetadata["provider"], number> = { hardcover: 3, googlebooks: 2, openlibrary: 1 };
...
function chooseSeriesCompletionOverride(items: Book[]): SeriesCompletionOverride | undefined {
  return items
    .map((book) => book.seriesCompletionOverride)
    .filter((value): value is SeriesCompletionOverride => Boolean(value))
    .sort((a, b) => (b.updatedAt ?? "").localeCompare(a.updatedAt ?? ""))[0];
}

function seriesPositions(value?: string): number[] {
  if (!value?.trim()) return [];
  const clean = value.trim().replace(/[–—]/g, "-").replace(/^#\s*/, "");
  const single = clean.match(/^\d+(?:\.\d+)?$/);
  if (single) return [Number(clean)];

  const range = clean.match(/^(\d+)\s*-\s*(\d+)$/);
  if (range) {
    const start = Number(range[1]);
    const end = Number(range[2]);
    if (start > 0 && end >= start && end - start <= 50) return Array.from({ length: end - start + 1 }, (_, index) => start + index);
  }
...
function seriesBookSort(a: SeriesCatalogBook, b: SeriesCatalogBook): number {
  const left = Number(a.position); const right = Number(b.position);
  if (Number.isFinite(left) && Number.isFinite(right)) return left - right || a.title.localeCompare(b.title);
  if (Number.isFinite(left)) return -1;
  if (Number.isFinite(right)) return 1;
  return String(a.position ?? "").localeCompare(String(b.position ?? "")) || a.title.localeCompare(b.title);
}

function seriesEntryPreference(entry: SeriesCatalogBook, libraryBooks: Book[]): number {
  let score = 0;
  const title = normalizeSeriesText(entry.title);
  const author = normalizeSeriesText(entry.author);
  if (title && libraryBooks.some((book) => normalizeSeriesText(book.title) === title)) score += 100;
  if (author && libraryBooks.some((book) => normalizeSeriesText(book.author) === author || (book.additionalAuthors ?? []).some((name) => normalizeSeriesText(name) === author))) score += 30;
  if (/^[\x00-\x7F]*$/.test(entry.title)) score += 5;
  if (entry.isbn) score += 2;
  return score - Math.min(20, entry.title.length / 20);
}

function inferMainlineCountFromRepeatedPositions(rows: SeriesCatalogBook[]): number | undefined {
  const positions = rows.map((entry) => numericSeriesPosition(entry.position)).filter((value): value is number => Boolean(value && Number.isInteger(value) && value > 0 && value <= 500));
  if (positions.length < 4) return undefined;
  const unique = [...new Set(positions)].sort((a, b) => a - b);
  const max = unique.at(-1) ?? 0;
  if (max < 2 || max > 500) return undefined;
...
function cleanSeriesCatalog(catalog: SeriesMetadata | undefined, libraryBooks: Book[], override: SeriesCompletionOverride | undefined): SeriesCatalogBook[] {
  if (!catalog) return override?.manualBooks?.map((entry) => ({ ...entry })) ?? [];
  const unique = [...new Map(catalog.books.map((entry) => [entry.providerId, entry] as const)).values()];
  ...
  if (included.size === 0 && expected && expected > 0 && expected <= 500) {
    const byPosition = new Map<number, SeriesCatalogBook[]>();
    for (const entry of rows) {
      const position = numericSeriesPosition(entry.position);
      if (!position || !Number.isInteger(position) || position < 1 || position > expected) continue;
      const group = byPosition.get(position) ?? []; group.push(entry); byPosition.set(position, group);
    }
  ...
  if (included.size === 0) {
    ...
    for (const book of libraryBooks) {
      for (const position of seriesPositions(book.seriesVolume)) {
        if (!Number.isInteger(position) || position < 1 || position > 500) continue;
        if (expectedLimit && position > expectedLimit) continue;
        const providerId = `library:${book.id}:position:${position}`;
        if (existingPositions.has(position) || excludedPositions.has(position) || excluded.has(providerId)) continue;
  ...
  if (manual.length) {
    ...
    for (const entry of manual) if (!seen.has(entry.providerId)) { rows.push({ ...entry }); seen.add(entry.providerId); }
  }
  return [...rows].sort(seriesBookSort);
}

function catalogBookMatch(catalog: SeriesCatalogBook, books: Book[]): Book | undefined {
  if (catalog.isbn) {
...
export function seriesProgress(books: Book[]): SeriesProgress[] {
  ...
  for (const book of books) {
    ...
    if (book.seriesMetadata?.name?.trim()) current.name = book.seriesMetadata.name.trim();
  ...
  return [...groups.values()].map(({ name, books: items }) => {
    ...
    const gaps: number[] = [];
    if (volumes.length >= 2) for (let volume = volumes[0]; volume <= volumes[volumes.length - 1]; volume += 1) if (!volumes.includes(volume)) gaps.push(volume);
    ...
    const catalogBooks: SeriesCatalogStatus[] = cleanedCatalog.map((entry) => {
...
export interface LibraryFlowDatum { name: string; added: number; read: number; pages: number; }
export function libraryFlowByYear(books: Book[]): LibraryFlowDatum[] {
  const years = new Map<string, LibraryFlowDatum>();
  const ensure = (year: string) => { const row = years.get(year) ?? { name: year, added: 0, read: 0, pages: 0 }; years.set(year, row); return row; };
  for (const book of books) {
    const addedYear = yearOf(book.dateAdded);
    if (addedYear) ensure(String(addedYear)).added += 1;
  }
  for (const event of readEvents(books)) { const row = ensure(String(event.year)); row.read += 1; row.pages += event.pages; }
